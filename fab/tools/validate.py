#!/usr/bin/env python3
"""validate.py — schema and quality-bar validation for recipes.jsonl.

Usage:
    python3 tools/validate.py recipes.jsonl
    python3 tools/validate.py recipes.jsonl --strict   # gate releases
...
VALID_RECIPE_CLASS = {
    ...
    "thermoplastic-laser-ablation",
    "paper-microfluidics-patterning",
    "dlp-sla-enclosed-channels",
    "2pp-direct-write",
...
def main():
    ...
    if not args:
        sys.exit("usage: validate.py recipes.jsonl [--strict]")
    path = Path(args[0])
    if not path.exists():
    ...
    for lineno, line in enumerate(path.read_text().splitlines(), start=1):
        ...
        rc = entry.get("recipe_class")
        if rc and rc not in VALID_RECIPE_CLASS:
            errors.append(
                f"line {lineno} ({entry.get('id', '?')}): recipe_class `{rc}` not in taxonomy"
            )
