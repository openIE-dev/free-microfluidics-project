...
## Wiener Lab clinical-chemistry reagents and analyzers (1959)

- **id**: `wiener-lab-clinical-chemistry-reagent-and-analyzer-line`
- **corpus**: private
- **device class**: flow-controller
- **creator**: Wiener Laboratorios S.A.I.C., Rosario
- **disclosure**: Wiener Laboratorios S.A.I.C., Rosario, Argentina; Wiener Lab product catalog (clinical-chemistry reagent kits, the CB and CM series benchtop analyzers, immunoturbidimetric and ELISA kits, the Metrolab analyzer line)
- **ip status**: patented
- **prior art notes**: An automated clinical-chemistry analyzer line: syringe- and peristaltic-pump-metered aspiration and dispensing of microliter sample and reagent volumes into reaction cuvettes with photometric readout, plus ion-selective-electrode modules. Discloses analyzer-fluidics architecture (probe wash stations, carryover-minimizing flow paths, microvolume metering) and reagent formulations. Relevant prior art for automated liquid-handling fluidics in benchtop diagnostics and for immunoturbidimetric and enzymatic assay reagent design. ip_citations draft.
...
- **disclosure**: Vapourtec Ltd. Continuous flow chemistry systems. https://www.vapourtec.com
- **ip status**: patented
- **prior art notes**: Modular continuous-flow chemistry platform with HPLC pumps, heated reactors, mixers, and inline analytics. Used widely in pharmaceutical chemistry for hazardous-reagent and high-temperature reactions. Anticipates: flow-chemistry-as-a-platform commercial offering, and the systems integration that enables academic flow-chem groups to do production work.
...
- **device class**: flow-controller
- **creator**: Syrris Ltd.
- **disclosure**: Syrris (acquired by Asynt 2022). https://www.syrris.com
- **ip status**: patented
- **prior art notes**: Modular flow-chemistry platform offering smaller-scale variants than Vapourtec; particularly used in academic and pharmaceutical R&D. Architectural cousin of Vapourtec; together they cover most of the commercial pharma-flow-chemistry market.
...
## Curology compounded prescription skincare (2014)

- **id**: `curology-skincare`
- **corpus**: private
- **device class**: dispenser-pipettor
- **creator**: Curology, Inc.
...
## Klipper-Controlled Open-Source Syringe Pump (2022)

- **id**: `klipper-syringe-pump-firmware-port`
- **corpus**: open
- **device class**: pump-component
- **creator**: Open community (Machine Agency, Jubilee project, Klipper3d community)
- **disclosure**: Multiple community designs, including 'klipper-syringe' and 'Jubilee tool-changer syringe tool' on GitHub (e.g., https://github.com/machineagency/jubilee, https://github.com/klipper3d/klipper); Wong et al. demonstration in Reddit r/3Dprinting and HardwareX-adjacent community publications 2022-2024
- **ip status**: open-copyleft
- **prior art notes**: Discloses repurposing of a 3D-printer motion-control firmware stack (Klipper) to drive syringe pumps as if they were extruders on a printer. The 'pump' is configured in printer.cfg as one or more extruder kinematics, with rotation_distance set so each mm of stepper travel corresponds to a known syringe-barrel displacement (and thus volume). The Pi-class host runs Klipper, which sends step pulses to a low-cost MCU board over USB; pressure-advance, input-shaping, and trapezoidal motion planning all transfer directly to fluidic motion. Released under GPLv3 (Klipper) plus CC-BY/MIT for individual mechanical designs. Anticipates: any claim to 'using 3D-printer motion-control firmware to actuate a syringe-pump with motion-planning features (input shaping, pressure advance) applied to fluid delivery' or 'controlling a multi-syringe array as a multi-extruder 3D-printer'. Anticipates fleet management of pumps via Mainsail/Fluidd web UI as 'standard 3D printer endpoints'. Specifically prior art against any post-2022 'liquid handling robot using 3D printer firmware' patent.
