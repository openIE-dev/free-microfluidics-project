...
## Hudson Robotics SOLO and PlateMate liquid-handling robots (1996)

- **id**: `hudson-solo-platemate`
- **corpus**: private
- **device class**: dispenser-pipettor
- **creator**: Hudson Robotics
...
## Eppendorf epMotion automated pipetting system (2003)

- **id**: `eppendorf-epmotion`
- **corpus**: private
- **device class**: dispenser-pipettor
...
## Andrew Alliance / Andrew+ Scriptable Pipetting Robot (2014)

- **id**: `evastreams-andrew-alpha-script`
- **corpus**: open
- **device class**: dispenser-pipettor
- **creator**: Andrew Alliance SA (Geneva, Switzerland) / Waters Corporation
- **disclosure**: Andrew Alliance SA (Geneva), commercial release 2014; OneLab cloud protocol designer launched 2017; acquired by Waters Corporation 2020; product manuals at https://andrewalliance.com; Buchs lab review in Lab Chip 2018 (DOI 10.1039/C8LC00018B)
- **ip status**: patented
- **prior art notes**: Pertinent prior art for pipetting-robot designs because Andrew uses a fundamentally different mechanical paradigm: instead of an integrated pipette, the robot grips an off-the-shelf manual single-channel pipette by its body and depresses the plunger via a separate actuator, then ejects tips into the manual pipette's own ejector cone. Discloses: (a) gripper-actuated manual pipette interface, (b) 'domino' labware that self-calibrates via molded keying features rather than per-deck calibration, (c) OneLab cloud protocol designer producing portable JSON protocol files. Mostly proprietary, but a sub-set of the OneLab protocol schema is public. Listed here to flag as prior art that any 'open robot grips manual pipette' design must navigate. Specifically anticipates the architectural pattern of leveraging the consumable ecosystem of an existing manual pipette.
...
## Andrew Alliance Andrew+ pipetting robot (2014)

- **id**: `andrew-pipetting-robot`
- **corpus**: private
- **device class**: dispenser-pipettor
- **creator**: Andrew Alliance (Waters Corp.)
- **disclosure**: Andrew Alliance Andrew pipetting robot launch 2014; Andrew+ successor 2019; acquired by Waters 2020. https://www.waters.com/andrew/.
- **ip status**: patented
- **prior art notes**: Pipetting robot whose dispense head is a standard handheld electronic pipette: the robot grasps and operates a manual pipette through a custom interface, not a built-in syringe. Anticipates: robots that wrap commodity pipettes rather than re-implementing the dispense element, which is a categorically different cost / regulatory profile; cloud-authored pipetting protocols decoupled from instrument firmware. Important architectural prior art for any 'commodity-pipette-driven robot' claim.
...
## Ypsomed mylife YpsoPump Insulin Pump (2016)

- **id**: `ypsomed-ypsopump`
- **corpus**: private
- **device class**: pump-component
- **creator**: Ypsomed Holding AG
- **disclosure**: Ypsomed Holding AG annual report 2016/2017; CE Mark 2016; FDA 510(k) K221237 2022
- **ip status**: patented
