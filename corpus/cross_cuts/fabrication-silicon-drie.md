---
title: fabrication-silicon-drie
parent: Cross-cuts
layout: default
---
...
- **id**: `vaught-cloutier-1981-hp-thermal-bubble-jet`
- **corpus**: private
...
- **disclosure**: Branebjerg, J.; Gravesen, P.; Krog, J. P.; Nielsen, C. R. Fast mixing by lamination. Proc. IEEE MEMS '96, 1996, 441–446.
- **ip status**: public-domain
- **prior art notes**: Disclosed silicon-MEMS lamination mixers: stacking thin layers with offset inlets to create flow lamination as a passive mixing strategy. Architectural ancestor of subsequent split-and-recombine mixer designs, including the Stroock 2002 herringbone mixer. Mid-1990s Danish silicon-MEMS work is systematically under-cited in US prior art databases despite being technically equivalent to slightly later US disclosures.
...
## Advion TriVersa NanoMate chip-ESI source (2003)

- **id**: `advion-triversa-nanomate`
- **corpus**: private
- **device class**: lab-on-chip
- **creator**: Advion BioSciences (now Advion Interchim Scientific)
- **disclosure**: Advion BioSciences TriVersa NanoMate. https://www.advion.com
- **ip status**: patented
- **prior art notes**: Commercial chip-ESI source: a silicon nozzle-array chip with one ESI nozzle per sample replaces the conventional capillary ESI tip in MS workflows. Architecturally the commercial descendant of Ramsey 1996 chip-ESI; widely deployed for proteomics workflows.

## Deterministic lateral displacement particle separation (2004)
...
- **disclosure**: Huang, L. R.; Cox, E. C.; Austin, R. H.; Sturm, J. C. Continuous particle separation through deterministic lateral displacement. Science 2004, 304, 987–990. DOI: 10.1126/science.1094567
- **ip status**: patented
- **prior art notes**: Disclosed deterministic lateral displacement: a periodic pillar array shifts particles above a critical size laterally with each row, while smaller particles pass through. Anticipates the entire DLD field including blood cell separation, CTC capture, and exosome isolation. Subsequent commercial implementations (e.g., GPB Scientific) build directly on this geometry.
...
- **id**: `memjet-waterfall-mems-printhead`
- **corpus**: private
- **device class**: inkjet-printhead
- **creator**: Memjet (originally Silverbrook Research, Sydney, Australia)
...
## Genia Technologies (Roche) nanopore sequencing (2009)
...
- **id**: `hp-pagewide-a50-a53-hdna-printhead-array`
- **corpus**: private
- **device class**: inkjet-printhead
- **creator**: Hewlett-Packard / HP Inc.
- **disclosure**: HP PageWide Web Press technical white paper (hp.com PageWide Technology white paper, easternengineering.com PDF); HP press releases for A50 (2012) and A53 (2016 drupa)
- **ip status**: patented
- **prior art notes**: Discloses a high-density-nozzle-architecture (HDNA) thermal inkjet die at 2,400 npi with redundant nozzle columns assigned to the same color so that several different nozzles can fire each 600-dpi dot row, providing 4× to 8× nozzle redundancy and statistical missing-nozzle compensation. Anticipates: (a) explicit per-color redundancy in pagewide single-pass printheads as a quality-engineering primitive, (b) stitched-die page-spanning bar architecture (PageWide T-series), (c) dual-drop-weight per-color firing for tone reproduction without halftoning artifacts. Used as the core engine of HP PageWide Web Press, PageWide XL Pro 10000, and HP A50/A53-derived publication-quality presses.
